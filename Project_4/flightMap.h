...
public:
  ...
  // Pre-condition: originCity and destinationCity are the origin and
  // destination city, respectively Post-condition: returns true if a sequence
  // of flights exists
  //              from origin to destination; otherwise return false.
  //              Cities visited during the search are marked as visited
  //              in the flight map.
  bool IsPath(int origin, int destination);
  // Determines wether a sequence of flights exists between two cities

  int DisplayItinerary(int originIndex, int destinationIndex);
  // Displays the flight itinerary
...
private:
  ...
  vector<bool> visited; // newly added for this part: to check if a city has
                        // been visited before
};
